...
# make doors, chests all "permanent features" (pFeature)
# make flames, other stuff like that "temporary features" (tFeature)
...
def tryMove(x, y, xChange, yChange):
    if (isWall(x + xChange, y + yChange) or isElementInList(x + xChange, y + yChange, doorX, doorY) or isElementInList(x + xChange, y + yChange, chestX, chestY) or (exitLocationX == x + xChange and exitLocationY == y + yChange)):
        return False
    else:
        return True
...
def isWall(x, y):
    if (isElementInList(x, y, doorX, doorY)):
        return False
...
def generateDungeon():
    ...
    global roomWidth
    global roomHeight
    global exitLocationX
    global exitLocationY
    ...
    for i in range(0, dungeonRoomCount):
        ...
        if (i != 0):
            if (roomDir == 0):
                doorX.append(currentX + width)
                doorY.append(currentY)
            elif (roomDir == 1):
                doorX.append(currentX - width)
                doorY.append(currentY)
            elif (roomDir == 2):
                doorX.append(currentX)
                doorY.append(currentY + height)
            elif (roomDir == 3):
                doorX.append(currentX)
                doorY.append(currentY - height)
        ...
        if (random.randint(0, 10) > 7 and i < dungeonRoomCount - 1 and i > 0):
            chestX.append(currentX)
            chestY.append(currentY)

        prevWidth = width
        prevHeight = height

    exitLocationX = roomCenterX[len(roomCenterX)-1]
    exitLocationY = roomCenterY[len(roomCenterY)-1]
...
def generateFinalDungeon():
    ...
    tryMove(screenWidth/2, screenHeight/2)

    # move the exit wayyy off screen
    exitLocationX = 999
    exitLocationY = 999
    ...
    for i in range(screenHeight):
        for j in range(screenWidth):
            if (playerX == j and playerY == i):
                ...
                currentLine += "#"
            elif (isElementInList(j, i, doorX, doorY)):
                currentLine += "◘"
            elif (exitLocationY == i and exitLocationX == j):
                currentLine += "↓"
            elif (isElementInList(j, i, chestX, chestY)):
                currentLine += "◙"
            elif (i == 0):
                # drawing UI
...
def runGameLogic():
    ...
    if (playerX == exitLocationX and playerY == exitLocationY):
        nextFloor()
